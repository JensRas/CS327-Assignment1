...
static void io_redisplay_visible_monsters(dungeon *d)
{
    ...
    * will revisit this in the future to add the acceleration matrix.        */
    ...
    for (pos[dim_y] = -PC_VISUAL_RANGE; pos[dim_y] <= PC_VISUAL_RANGE; pos[dim_y]++) {
        for (pos[dim_x] = -PC_VISUAL_RANGE; pos[dim_x] <= PC_VISUAL_RANGE; pos[dim_x]++) {
            ...
            if ((illuminated = is_illuminated(d->PC, d->PC->position[dim_y] + pos[dim_y],
            ...
            if (d->character_map[d->PC->position[dim_y] + pos[dim_y]]
                ...
                switch (pc_learned_terrain(d->PC, d->PC->position[dim_y] + pos[dim_y],
                                        d->PC->position[dim_x] + pos[dim_x])) {
                case ter_wall:
...
static int compare_monster_distance(const void *v1, const void *v2)
{
    ...
    return (thedungeon->pc_distance[(*c1)->position[dim_y]][(*c1)->position[dim_x]] -
            thedungeon->pc_distance[(*c2)->position[dim_y]][(*c2)->position[dim_x]]);
}
...
void io_display(dungeon *d)
{
    ...
    clear();
    for (visible_monsters = -1, pos[dim_y] = 0; pos[dim_y] < DUNGEON_Y; pos[dim_y]++) {
...
uint32_t io_teleport_pc(dungeon *d)
{
    ...
    mvprintw(0, 0, "Choose a location.  'g' or '.' to teleport to; 'r' for random.");
...
static void io_list_monsters_display(dungeon *d, character **c, uint32_t count)
{
    ...
    if (count <= 13) {
        ...
        mvprintw(19, 9, " %-60s ", "");
        mvprintw(20, 9, " %-60s ", "Arrows to scroll, escape to continue.");
...
static void io_list_carry_slots_display(dungeon *d, uint32_t count)
{
    uint32_t i;
    char (*s)[60];
    char tmp[41];  /* 19 bytes for relative direction leaves 40 bytes *
    ...
    mvprintw(3, 9, " %-60s ", "");
    /* Borrow the first element of our array for this string: */
    snprintf(s[0], 60, "You are carrying %d items:", count);
    mvprintw(4, 9, " %-60s", s);
    mvprintw(5, 9, " %-60s ", "");
    ...
    mvprintw(10 + 6, 9, " %-60s ", "");
    mvprintw(10 + 7, 9, " %-60s ", "Hit escape to continue.");
    while (getch() != 27 /* escape */);

    free(s);
}

static void io_list_carry_slots(dungeon *d)
{
    /* Display it */
    io_list_carry_slots_display(d, d->PC->carry_slots_held);
